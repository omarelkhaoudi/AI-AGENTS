...
const SPECIALIZED_AGENT_IDS = Object.freeze([
  "finance",
  "commercial",
  "production",
  "purchasing",
...
const SPECIALIZED_TOOL_IDS = Object.freeze([
...
const MVP_AGENT_IDS = Object.freeze([
  "director",
...
test("MVP central scenario orchestrates the leader request across all specialized agents offline", async (t) => {
  ...
  assert.deepEqual(plan.steps.map((step) => step.agentId), SPECIALIZED_AGENT_IDS);
  assert.deepEqual(plan.steps.map((step) => step.toolName), SPECIALIZED_TOOL_IDS);
  assert.deepEqual(plan.steps.map((step) => step.sequence), [1, 2, 3, 4, 5, 6, 7, 8]);
  assert.equal(request.executions.length, 8);
  ...
  assert.equal(request.result.summary.completedExecutions, 8);
  assert.deepEqual(request.result.summary.agents, SPECIALIZED_AGENT_IDS);
  ...
  assert.equal(events.filter((type) => type === "permission_checked").length, 8);
  assert.equal(events.filter((type) => type === "tool_called").length, 8);
  assert.equal(events.filter((type) => type === "execution_completed").length, 8);
});
...
test("POST /api/director/requests returns a clean consolidated demo response", async (t) => {
  ...
  assert.match(body.summary.headline, /Point complete: 8\/8 agents responded/);
  assert.deepEqual(body.agents.map((agent) => agent.id), MVP_AGENT_IDS);
  assert.deepEqual(body.agents.slice(1).map((agent) => agent.tool), SPECIALIZED_TOOL_IDS);
  ...
  assert.equal(body.audit.filter((event) => event.type === "permission_checked").length, 8);
  assert.equal(body.audit.filter((event) => event.type === "tool_called").length, 8);
  assert.equal(body.audit.filter((event) => event.type === "execution_completed").length, 8);
  ...
  const scenarios = [
    {
      label: "A",
      message: "Fais-moi le point complet de l'entreprise aujourd'hui.",
      agents: SPECIALIZED_AGENT_IDS,
      assertBody: (body) => {
        assert.equal(body.summary.whatIsGoingWell.length > 0, true);
        assert.equal(body.summary.delayed.length > 0, true);
        assert.equal(body.summary.receivables.length > 0, true);
        assert.equal(body.summary.purchaseNeeds.length > 0, true);
        assert.equal(body.summary.blockers.length > 0, true);
...
test("POST /api/director/requests reports partial results when Marketing is unavailable", async (t) => {
  ...
  assert.match(body.summary.headline, /7 agent\(s\) out of 8/);
  assert.match(body.summary.headline, /marketing/);
  assert.equal(body.results.filter((result) => result.status === "completed").length, 7);
...
test("POST /api/director/requests reports partial results when an agent is unavailable", async (t) => {
  ...
  const { body, response } = await postDirector(app, "Fais-moi le point complet de l'entreprise aujourd'hui.");
  const marketing = body.results.find((result) => result.agent === "marketing");
